...
import { useUpdateSchedule, useDisplays, useGroups } from '../hooks/useApi';
...
export const EditScheduleModal: React.FC<EditScheduleModalProps> = ({ isOpen, onClose, schedule }) => {
  ...
  const parseCronExpression = (cron: string) => {
    ...
    if (parts.length !== 5) {
      return { hour: '08', minute: '00', weekdays: defaultWeekdays };
    }
    ...
    const weekdays = { ...defaultWeekdays };
    
    if (dayOfWeek === '*') {
      // All days selected
      Object.keys(weekdays).forEach(day => {
        weekdays[day as keyof typeof weekdays] = true;
      });
    } else {
      // Parse specific days
      ...
      selectedDays.forEach(dayNum => {
        ...
        if (dayName) {
          weekdays[dayName as keyof typeof weekdays] = true;
        }
      });
    }
  ...
  // Initialize form with schedule data when modal opens
  useEffect(() => {
    if (schedule && isOpen) {
      const { hour, minute, weekdays } = parseCronExpression(schedule.cron_expression);
      setFormData({
        name: schedule.name,
        hour,
        minute,
        action: schedule.action,
        display_ids: schedule.display_ids || [],
